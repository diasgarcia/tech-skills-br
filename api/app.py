...
DESCRIPTION = """
### 🔬 Mapeamento de Skills em Tecnologia no Brasil (projeto de dados)

API pública de consulta e análise quantitativa de dados sobre o mercado de trabalho em computação e tecnologia no Brasil, comparando a demanda real das empresas com os referências técnicas do **fontes públicas (referências)** e da **referências técnicas**.
...
app = FastAPI(
    title="Mapeamento de Skills em Tecnologia no Brasil (projeto)",
    description=DESCRIPTION,
    version=__version__,
    contact={"name": "Projeto de dados - Mercado vs. referências técnicas"},
    lifespan=lifespan,
)
...
def raiz() -> dict:
    return {
        "nome": "Mapeamento de Skills em Tecnologia no Brasil - API de Pesquisa (projeto)",
        "versao": __version__,
